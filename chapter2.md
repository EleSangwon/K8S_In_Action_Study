# Chapter2 - 도커와 쿠버네티스의 첫걸음 Page 67 ~ 108
...
```
kubectl scale 명령어로, 파드의 레플리카 개수를 늘릴 수 있다.

파드를 expose하여 서비스 객체를 만들었다고 가정해보자.
하나의 파드에서 같은 기능을 하는 파드가 3개로 늘어난다면, 

curl <서비스URL>:포트 를 실행하면 항상 같은 앱 인스턴스를 사용할까 ?

결론은 무작위로 3개의 파드에 접근한다. 

쿠버네티스는 하나 이상의 파드 인스턴스가 서비스 뒤에 존재할 떄 
여러 파드 앞에 로드 밸런서 역할을 한다.

```
![캡처18](https://user-images.githubusercontent.com/50174803/127981936-3ab460a7-a4da-4fae-889e-2842bcf76d23.jpg)
